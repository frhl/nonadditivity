...
readonly bashscript_local="_conditional_add.sh"
readonly bashscript_remote="${remote_dir}/_conditional_add.sh"
...
readonly plink_dir="/mnt/project/wes_ko_ukbb/data/conditional/combined_final/v2"
readonly plink_file="${plink_dir}/UKB.wes.merged.phased.qc_final.${pop}.af${af}.popmax.pp${pp}.${group}.ANNO.${mode}.auto.gel_imputed.maf${maf}.chrCHR.v2"
#readonly out_dir="/wes_ko_ukbb/data/conditional/output/31jan2025_fix"
readonly out_dir="/wes_ko_ukbb/data/conditional/output/additive/v2"
...
while IFS=$'\t' read -r \
   ...
   # skip header
    if [[ "${annotation}" != "pLoF" ]]; then
      continue
    fi

    echo "manually setting synonymous!" 
    ...
    p_value_decimal=$(echo "$p_value_dom" | awk '{printf "%.10f\n", $1}')
    if (( $(echo "$p_value_decimal > $p_dom_threshold" | bc -l) )); then
        continue
    fi
    ...
    plink_file_chr=${plink_file/CHR/${chromosome}}
    plink_file_chr_anno=${plink_file_chr/ANNO/${annotation}}
    ...
    out_prefix="nearby_common_conditional.maf_cutoff=${maf}.pop=${anc}.trait=${pheno}.symbol=${hgnc_symbol}.transcript=${ensembl_transcript_id}.chrom=${chromosome}.group=${group}.anno=${annotation}.mode=${mode}"
    out="${out_dir}/${out_prefix}.txt.gz"

    if [[ $(dx_file_exists $(wo_mnt_project "${plink_file_chr_anno}.bed")) -eq "1" ]]; then
        ...
        dx run app-swiss-army-knife \
          -icmd="
            bash /mnt/project/${bashscript_remote} \
              ...
              --plink_file '${plink_file_chr_anno}' \
          ...
          --folder=".${out_dir}" \
          --priority high \
          --name "cond_add_${pheno}_${hgnc_symbol}" -y
      else
      ...
      >&2 echo "${plink_file_chr_anno} does not exists. Skipping.."
    fi
