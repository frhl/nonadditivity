...
readonly phenos_path="${pheno_dir}/ukb_cts_phenotypes.inst0.dec2024.with_covariates.txt.gz"
readonly phenos=$(wo_mnt_project "${pheno_dir}/ukb_cts_phenotypes.inst0.dec2024.phenos")
...
for pop in "eur"; do
  out_dir="/wes_ko_ukbb/data/lrt_testing/lrt_unrel/2df_chi2_trick_final"
  dx mkdir -p ${out_dir}
  for pp in  "0.90"; do
    for af in "05"; do
      echo "${phenos}"
