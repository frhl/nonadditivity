...
calc_hwe_pvalue <- function(g) {
  # Count observed genotypes
  n <- length(g)
  n0 <- sum(g == 0)  # aa (homozygous ref)
  n1 <- sum(g == 1)  # Aa (heterozygous)
  n2 <- sum(g == 2)  # AA (homozygous alt)
  
  # Estimate allele frequency from data
  p_hat <- (2 * n2 + n1) / (2 * n)
  q_hat <- 1 - p_hat
...
dthwe <- all_results[all_results$F_Val==0.05,]
sum(dthwe$HWE_Pval<1E-10)/nrow(dthwe)
