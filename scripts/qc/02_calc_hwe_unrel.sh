...
for CHR in {1..22} X; do
  ...
  #for pop in "eur"; do
    ...
    out_prefix="UKB.wes.chr${CHR}.phased.full_qc.${pop}.unrelated.hardy"
    ...
    if [[ $(dx_file_exists "${out_dir}/${out_prefix}.frqx.gz") -eq 0 ]]; then
      dx run app-swiss-army-knife \
        -icmd="
          ...
          plink2 --bfile tmp --threads ${threads} --hwe 1e-9 --write-snplist --out ${out_prefix} &&
          rm ${out_prefix}.log &&
