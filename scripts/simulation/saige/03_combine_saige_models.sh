...
# note: this script combines additive, recessive, and dominance SAIGE results into a single file

readonly rscript_local="_combine_saige_models.R"
readonly rscript_remote="wes_ko_ukbb/scripts/simulation/saige/_combine_saige_models.R"
...
# Define paths
readonly step2_dir="/wes_ko_ukbb/data/saige/step2/simulated/revisions/051025"
readonly out_dir="/wes_ko_ukbb/data/saige/step2/simulated/revisions/051025/combined"
  ...
  for h2 in "${heritabilities[@]}"; do
    ...
    for rep in $(seq 1 ${n_reps}); do
      ...
      for anno in "${annotations[@]}"; do
        ...
        # Define input files
        add_file="${step2_dir}/UKB.auto.qc_final.${pheno}.additive.${anc}.af${af}.pp${pp}.${anno}.txt.gz"
        rec_file="${step2_dir}/UKB.auto.qc_final.${pheno}.recessive.${anc}.af${af}.pp${pp}.${anno}.txt.gz"
        dom_file="${step2_dir}/UKB.auto.qc_final.${pheno}.dominance.${anc}.af${af}.pp${pp}.${anno}.txt.gz"

        # Output file
        out_prefix="saige_step2.${anc}.${pheno}.${anno}.combined"
        out_file="${out_dir}/${out_prefix}.tsv.gz"
            ...
            dx run app-swiss-army-knife \
              ...
              -icmd="
                Rscript /mnt/project/${rscript_remote} \
                  --additive UKB.auto.qc_final.${pheno}.additive.${anc}.af${af}.pp${pp}.${anno}.txt.gz \
                  --recessive UKB.auto.qc_final.${pheno}.recessive.${anc}.af${af}.pp${pp}.${anno}.txt.gz \
                  --dominance UKB.auto.qc_final.${pheno}.dominance.${anc}.af${af}.pp${pp}.${anno}.txt.gz \
              ...
              --destination "${out_dir}" \
              -y \
              --name "combine_saige_${anc}_${pheno}_${anno}"
