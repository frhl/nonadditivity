...
main <- function(opt)
{
    ...
    cat("=============================================================\n")
    cat("Creating QQ plots from combined SAIGE results\n")
    cat("=============================================================\n\n")

    # List all combined files matching ancestry and annotation
    pattern <- paste0(".*", opt$ancestry, ".*", opt$annotation, "\\.combined\\.tsv\\.gz$")
...
option_list = list(
    make_option(c("-c", "--combined_dir"), type="character", default=NULL,
        help="directory containing combined SAIGE files", metavar="character"),
