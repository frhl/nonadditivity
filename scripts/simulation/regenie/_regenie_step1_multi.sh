#!/usr/bin/env bash
# Worker script that runs REGENIE step1 for ALL phenotypes at once
...
KEEP="$3"               # Keep file (sample IDs to include)
EXTRACT="$4"            # Extract file (variant IDs to include)
PHENOCOLLIST="$5"       # Comma-separated phenotype list
...
echo "Running REGENIE step1 for ALL phenotypes with:"
echo "  BFILE: ${BFILE}"
...
echo ""
echo "All column names in phenotype file:"
head -1 "${PHENOFILE_LOCAL}" | tr ' ' '\n' | nl

echo ""
echo "Phenotypes to fit: ${PHENOCOLLIST}"
echo "Covariates specified: ${COVARCOLLIST}"
echo "Categorical covariates specified: ${CATEGCOVARCOLLIST}"
...
# Run REGENIE step 1 for ALL phenotypes
# Key: Use --phenoColList to explicitly specify which columns are phenotypes
# This prevents REGENIE from treating other columns (like year_of_birth, etc.) as phenotypes
echo "Running REGENIE step1 for specified phenotypes..."
...
echo ""
echo "Number of phenotypes fitted:"
grep -c "phenotype" ${OUT}.log || echo "Check log file for phenotype count"
