...
main <- function(opt) {
    ...
    for (i in seq_along(files)) {
        ...
        tryCatch({
            ...
            # REGENIE combined files should have .P columns (already converted from LOG10P in combine step)
            # But if they are raw REGENIE files, they might be LOG10P
            
            # Check for standard P-value columns first
            p_cols <- grep("\\.P$|^p\\.value$", colnames(dt), value = TRUE)
            
            # If no standard P columns, look for LOG10P and convert
            if (length(p_cols) == 0) {
                log_cols <- grep("LOG10P", colnames(dt), value = TRUE)
                if (length(log_cols) > 0) {
                    cat("  Converting LOG10P columns to P-values...\n")
                    for (lc in log_cols) {
                        new_col <- sub("LOG10P", "P", lc)
                        dt[[new_col]] <- 10^(-dt[[lc]])
                        p_cols <- c(p_cols, new_col)
                    }
                }
            }
