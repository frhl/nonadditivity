...
for CHR in 18; do
  ...
  dx run app-swiss-army-knife -icmd="
    ...
    qctool -g ${ref_bgen} -s ${ref_sam} -filetype bgen -incl-samples ${keep_samples} -excl-samples ${exclude_samples} -incl-positions pos.txt -ofiletype binary_ped -og tmp &&
    plink2 --bfile tmp --output-chr chrMT --recode vcf bgz --out ${out_prefix} &&
    rm -f ${out_prefix}.log &&
    rm -f tmp* &&
    rm -f pos.txt
  "\
